...
########################################################################################
# covid-qa csvs
COVID_QA_DATA = ('%s' % (os.path.join(os.path.dirname(__file__), './community.csv')))
COVID_QA_NEWS_DATA = ('%s' % (os.path.join(os.path.dirname(__file__), './news.csv')))
COVID_QA_MULTILINGUAL_DATA = (
        '%s' % (os.path.join(os.path.dirname(__file__), './multilingual.csv'))
)
...
class CovidQAChatbot(object):
    """COVID-19 Information chat bot """
    def __init__(self):
        log.debug(f'Entering: "{inspect.currentframe().f_code.co_name}"')
        self.set_dafa_frame_from_csv(
            [COVID_QA_DATA, COVID_QA_NEWS_DATA, COVID_QA_MULTILINGUAL_DATA]
        )
        self.load_model()
    ...
    def set_dafa_frame_from_csv(self, csvs=None):
        ...
        columns_to_keep = ['title', 'question', 'answer', 'wrong_answer', 'url', 'source']
        df_list = []
...
def main():
    ...
    log.debug(f'Entering: "{inspect.currentframe().f_code.co_name}"')
    ml = CovidQAChatbot()
    ml.print_chatbot_ready_text('COVID-19 CHATBOT BY AMITESH')
    welcome_text = 'Hi, I am COVID-19 bot. You can ask me any questions related to covid-19!!!'
